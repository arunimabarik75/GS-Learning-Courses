...
static void treeFactory(GenericTree<int> &tree)
{
  ...
  // first level - 8 and 15
  auto *eight = rootPtr->addChild(8);
  auto *fifteen = rootPtr->addChild(15);

  // second level - 16 and 23
  auto *sixteen = eight->addChild(16);
  auto *twentyThree = eight->addChild(23);

  // third level - 42
  auto *fortyTwo = sixteen->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  while (!nodes.empty())
  {
    ...
    for (auto childPtr : curr->childrenPtrs)
    {
      nodes.push(childPtr);
    }
  }

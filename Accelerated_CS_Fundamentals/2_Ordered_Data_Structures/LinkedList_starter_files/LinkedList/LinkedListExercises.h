...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  // Empty linked list or smallest element -> insert at front
  ...
  // New element is greatest element - insert at back
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T> &other) const
{
  ...
  Node *head1 = left.getHeadPtr();
  Node *head2 = right.getHeadPtr();

  while (head1 != NULL and head2 != NULL)
  {
    if (head1->data <= head2->data)
    {
      merged.pushBack(head1->data);
      head1 = head1->next;
    }
    else
    {
      merged.pushBack(head2->data);
      head2 = head2->next;
    }
  }
